...
def main(
    ...
    with jnp.load('mnist.npz') as datafile:
        x_train = jnp.array(datafile['x_train']) / 255
        x_test = jnp.array(datafile['x_test']) / 255
        y_train = jnp.array(datafile['y_train'])
        y_test = jnp.array(datafile['y_test'])

    cross_entropy_value_and_grad = jax.value_and_grad(cross_entropy)
    ...
    for step in tqdm.trange(num_steps, dynamic_ncols=True):
        ...
        loss, grads = cross_entropy_value_and_grad(model, x_batch, y_batch)
